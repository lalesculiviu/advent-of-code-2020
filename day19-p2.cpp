...
bool rec(int nrule, int start, int end)
{
	...
	if(a[nrule][start][end]>=0){
		...
		if(leftPossibility[nrule].size()==3){
			assert(leftPossibility[nrule].size()==3);
			int p1=leftPossibility[nrule][0];
			int p2=leftPossibility[nrule][1];
			int p3=leftPossibility[nrule][2];
			for(int q=1; q<end-start-1; q++)
				for(int r=q+1; r<end-start; r++)
					if(rec(p1, start, start+q) && rec(p2, start+q, start+r) && rec(p3, start+r, end)){
						//cout<<"a[nrule][start][end]="<<a[nrule][start][end]<<", returning 1"<<endl;
						return (a[nrule][start][end]=1);
					}
		}
		else if(leftPossibility[nrule].size()==2){
		...
		if(rightPossibility[nrule].size()>0){
			if(rightPossibility[nrule].size()==3){
				assert(rightPossibility[nrule].size()==3);
				
				int p1=rightPossibility[nrule][0];
				int p2=rightPossibility[nrule][1];
				int p3=rightPossibility[nrule][2];
				for(int q=1; q<end-start-1; q++)
					for(int r=q+1; r<end-start; r++)
						if(rec(p1, start, start+q) && rec(p2, start+q, start+r) && rec(p3, start+r, end)){
							//cout<<"a[nrule][start][end]="<<a[nrule][start][end]<<", returning 1"<<endl;
			...
			else if(rightPossibility[nrule].size()==2){
